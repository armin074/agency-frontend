<template>
    <div>
      ...
      <form @submit.prevent="sendEmail">
        <h1>Kontaktieren Sie uns noch heute, um mehr darüber zu erfahren, wie wir Ihnen helfen können.</h1>
        <label for="name">Ihre Name:</label>
        <input type="text" id="name" v-model="name"  required />
        <label for="email">Ihre E-Mail Adresse:</label>
        <input type="email" id="email" v-model="email" required />
        <label for="number">Ihre Telefon Nummer:</label>
        ...
        <button type="submit" :disabled="isSending">
          ...
          <span v-else>Email Senden</span>
        </button>
       
        <div v-if="isEmailSent" class="success-message">
            E-Mail erfolgreich gesendet, wir werden uns so schnell wie möglich bei Ihnen melden, vielen Dank für Ihre Anfrage.
        </div>
      </form>
    </div>
  </template>
